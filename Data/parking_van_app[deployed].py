...
if grid_response:
        if grid_response.selected_data is not None and not grid_response.selected_data.empty: 
            ...
            walking_text = f"{round((distance_km * 10))} minute walk"
            distance_text = f"{distance_km}km away"
            line = folium.PolyLine(locations=[user_coords, target_coords2], weight=4, dash_array="5", popup= walking_text, tooltip= distance_text).add_to(m)
